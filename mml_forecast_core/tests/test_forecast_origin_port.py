...
def test_forecast_origin_port_has_no_name_get():
    """name_get() must not exist — removed in Odoo 18."""
    src_path = os.path.join(os.path.dirname(__file__), '..', 'models', 'forecast_origin_port.py')
    with open(src_path, encoding='utf-8') as f:
        source = f.read()
    tree = ast.parse(source)
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and node.name == 'name_get':
            pytest.fail("forecast_origin_port.py still defines name_get() — must use _compute_display_name()")


def test_forecast_origin_port_has_compute_display_name():
    ...
    pytest.fail("_compute_display_name() not found in forecast_origin_port.py")
