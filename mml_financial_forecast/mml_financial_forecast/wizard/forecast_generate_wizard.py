...
class ForecastGenerateWizard(models.TransientModel):
    ...
    def _get_demand_forecast(self, config, months):
        ...
        Strategy:
            1. Try ROQ forecast model if available
            2. Fallback: use trailing 12-month sale order lines as proxy
        """
        demand = []

        # --- Strategy 1: ROQ module ---
        # Uncomment when mml_roq module provides forecast data
        # roq_model = self.env.get('roq.forecast.line')
        # if roq_model:
        #     return self._demand_from_roq(config, months, roq_model)
    ...
    def _generate_cashflow_lines(self, config, months, revenue_lines, cogs_lines):
        ...
        month_starts = [m[0] for m in months]
        ...
        for cogs in cogs_lines:
            ...
            if m in month_map:
                ...
                # Duty + GST (15% on CIF value as import GST estimate)
